"""
Full fine-tune Qwen2.5-0.5B (base) for Hindi <-> Marathi translation via
supervised instruction fine-tuning (SFT) on Qwen's ChatML template.

Adapted from qwen_finetune/Bhili/qwen_finetune.py -- same recipe, two
...
generation on val + test, same as the manual beam pass in the mT5/NLLB
scripts.
...
def run_direction(language, direction, train_csv, val_csv, test_csv, epochs,
    ...
    # global batch under DDP is per_device_bs * grad_accum * world_size.
    world_size = dist.get_world_size() if dist.is_available() and dist.is_initialized() else 1
    if already_trained:
        warmup_steps = 0  # training is skipped entirely in this case
    else:
        steps_per_epoch = -(-len(tokenized_train) // (8 * 4 * world_size))  # ceil div
    ...
    if trainer.is_world_process_zero():
        ...
        _, _, val_bleu, val_chrf = generate_and_score(
            ...
            model, tokenizer, device, num_beams=2)
