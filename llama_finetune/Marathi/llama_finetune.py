"""
Full fine-tune Llama-3.1-8B (base) for Hindi <-> Marathi translation via
supervised instruction fine-tuning (SFT) on Llama-3's chat format.

Adapted from llama_finetune/Bhili/llama_finetune.py -- same recipe, two
differences specific to Marathi's data:
  - No English column (RECAP/datasets/Marathi/*.csv is unique_id,Hindi,
    Marathi only) -- en2tgt/tgt2en are dropped, only hi2tgt/tgt2hi are
    supported (same scope already used for Marathi in indictrans2_finetune/
    and sarvam_finetune/).
  - _load_and_clean() only requires {Hindi, Marathi}, not English too.
...
formatted as a chat turn using Llama-3's special tokens:
    ...
    Translate the following text from Hindi to Marathi:
    <source text><|eot_id|><|start_header_id|>assistant<|end_header_id|>

    <target text><|eot_id|>

IMPORTANT: like qwen_finetune.py's base checkpoint, Llama-3.1-8B's base
tokenizer does NOT ship a `chat_template` (verified directly against the
real tokenizer -- `tokenizer.chat_template is None` on the base model). The
format above is built manually in build_prompt_text()/build_full_text()
rather than via tokenizer.apply_chat_template(), matching
llama_finetune/Bhili/llama_finetune.py exactly.
...
recipe for instruction-following LLMs, matching qwen_finetune.py.
...
Checkpoint selection is by eval_loss (perplexity) -- plain Trainer doesn't
generate during periodic eval. Real BLEU/chrF++ are computed once at the
end via actual generation on val + test, same as qwen_finetune.py.

direction(s), data paths, and epochs are read from config.json (same
directory as this script):
    {
      "language": "marathi",
      "direction": ["hi2tgt", "tgt2hi"],
      "epochs": 3,
...
def _load_and_clean(csv_file: str, tgt_column: str) -> pd.DataFrame:
    ...
    needed = {"Hindi", tgt_column}
    assert needed.issubset(df_raw.columns), \
        f"Expected columns {needed} in {csv_file}, got {list(df_raw.columns)}"

    df = df_raw[["Hindi", tgt_column]].dropna()
    for col in ["Hindi", tgt_column]:
        df[col] = df[col].astype(str).str.strip()
    df = df[(df["Hindi"] != "") & (df[tgt_column] != "")].reset_index(drop=True)
    print(f"[Data] Usable parallel rows (both langs present): {len(df)}")
    return df
...
def run_direction(language, direction, train_csv, val_csv, test_csv, epochs,
    ...
    assert direction in ["hi2tgt", "tgt2hi"], \
        f"Unknown direction '{direction}' in config.json, choices: hi2tgt, tgt2hi"
    ...
    output_dir = f"./llama-{language}-{direction}-finetuned{suffix}"
    ...
    candidate_cols = ["Hindi", tgt_column, "unique_id", "Unique_ID"]
    ...
    if already_trained:
        ...
        # overflow, see llama_finetune/Bhili/llama_finetune.py's identical
        # comment for the full story. Always training fresh is slower on a
        # re-run but reliable; the "already_trained" check above still avoids
        # wasting a full retrain on directions that already finished.
    ...
    if trainer.is_world_process_zero():
        ...
        # ---- Real generation-based BLEU/chrF++ on val + test (beam=2) ----
        print("\n=== Generating on validation set (beam=2) ===")
        ...
        print(f"\n=== FINAL TEST SCORES [{language}/{direction}] (beam=2) ===")
